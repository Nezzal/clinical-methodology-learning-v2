...
<body>
  ...
  const cleanContentForBubble = (content: string) => {
    let clean = content.replace(/<params_synthese>([\s\S]*?)<\/params_synthese>/g, '');
    clean = clean.replace(/<params_strobe>([\s\S]*?)<\/params_strobe>/g, '');
    return clean.trim();
  };
  ...
  const getStrobeParamsFromMessage = (content: string) => {
    const match = content.match(/<params_strobe>([\s\S]*?)<\/params_strobe>/);
    if (match && match[1]) {
      try {
        return JSON.parse(match[1].trim());
      } catch (e) {
        console.warn("Failed to parse STROBE JSON:", e);
      }
    }
    return null;
  };
  ...
  const suggestions = chatMode === 'protocol'
    ...
    : chatMode === 'strobe'
    ? [
        "Aidez-moi à rédiger le résumé structuré.",
        "Quels sont les biais potentiels pour une étude transversale ?",
        "Comment structurer la partie méthodes selon STROBE ?",
        "Générer la synthèse finale <params_strobe> pour le générateur."
      ]
    : [
          ...
          {chatMode === null ? (
              ...
              <div className={styles.modeCards}>
                ...
                <div 
                  ...
                  onClick={() => {
                    ...
                    if (user) {
                      saveFirestoreChat(user.uid, newSessionId, "Rédaction d'article STROBE", [welcome], 'strobe')
                        .catch(e => console.error("Erreur de sauvegarde projet init:", e));
                      ...
                      setSessions(prev => {
                        ...
                        return [
                          {
                            id: newSessionId,
                            title: "Rédaction d'article STROBE",
                            messages: [welcome],
                            mode: 'strobe',
                            createdAt: new Date().toISOString(),
                            updatedAt: new Date().toISOString()
                ...
                >
                  <div className={styles.modeIcon} style={{ color: 'var(--accent-secondary)' }}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
                      <polyline points="14 2 14 8 20 8" />
                      <line x1="16" y1="13" x2="8" y2="13" />
                  ...
                  <h3 className={styles.modeCardTitle}>Rédaction d'Article (STROBE)</h3>
                  <p className={styles.modeCardDesc}>
                    Préparez votre article scientifique selon les critères de la grille STROBE. Idéal pour structurer vos résultats observationnels et les exporter.
                  </p>
                </div>
              </div>
            </div>
            ...
            <>
              ...
              <div className={styles.messagesList} ref={messagesListRef}>
                {messages.map((msg, index) => (
                  ...
                  >
                    ...
                    {msg.role === 'assistant' && msg.content.includes('<params_strobe>') && (() => {
                      const params = getStrobeParamsFromMessage(msg.content);
                      if (!params) return null;
                      ...
                      return (
                        <div className={styles.synthesisContainer}>
                          <div className={styles.synthesisHeader}>
                            ...
                            <span>Bilan STROBE compilé !</span>
                          </div>
                          <div className={styles.synthesisDesc}>
                            Les paramètres de votre article d'étude observationnelle (<b>{params.title || 'Sans titre'}</b>) ont été extraits selon la grille STROBE par le tuteur.
                          </div>
                          <button 
                            type="button" 
                            className={styles.transferBtn}
                            onClick={async () => {
                              localStorage.setItem('recif_imported_strobe_params', JSON.stringify(params));
                              if (user && activeSessionId) {
                                const existingSession = sessions.find(s => s.id === activeSessionId);
                                const currentTitle = existingSession?.title || 'Discussion STROBE';
                                try {
                                  await saveFirestoreChat(user.uid, activeSessionId, currentTitle, messages, chatMode || undefined);
                                } catch (e) {
                                  console.error("Erreur de sauvegarde finale avant redirection:", e);
                              ...
                              router.push('/article?import=direct');
                            }}
                          >
                            <span>Transférer vers le Rédacteur d'Article</span>
                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                              <line x1="5" y1="12" x2="19" y2="12" />
                              <polyline points="12 5 19 12 12 19" />
                            </svg>
              ...
              >
                ...
                >
                  <span className={styles.sendBtnText}>Envoyer</span>
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
